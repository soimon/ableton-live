...
export class Properties<GP, CP, TP, SP, OP> {
    ...
    constructor(
        ...
        protected childrenInitialProps?: Partial<{
            [T in keyof CP]: (string | ChildrenInitialProps)[];
        }>,
    ...
    async get<T extends keyof GP>(
        ...
        const result = await this.ableton.get(
            this.path,
            prop as string,
            this._id
        );
    ...
    async children<T extends keyof CP>(
        child: T,
        childProps?: string[],
        index: number?
    ): Promise<T extends keyof TP ? TP[T] : CP[T]> {
        let initialProps;
        ...
        const result = await this.ableton.getChildren(
            this.path,
            { child: child as string, initialProps, index },
            this._id
        );
    ...
    async child<T extends keyof CP>(
        child: T,
        index: number,
        childProps?: string[]
    ): Promise<(T extends keyof TP ? TP[T] : CP[T])[number] | undefined> {
    ...
    async observe<T extends keyof OP | keyof CP>(
        prop: T,
        listener: (
            data: T extends keyof OP ? OP[T] : T extends keyof TP ? TP[T] : any
        ) => any
    ): Promise<any> {
